...
import PredictiveRestCalculator from './PredictiveRestCalculator';
...
export default function CrewConflictIntelligence() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Modern Segmented Tab Bar Switcher */}
      <div className="flex p-1 bg-gray-100 dark:bg-gray-900 rounded-2xl w-fit border border-gray-200 dark:border-gray-800 select-none">
        <button
          onClick={() => setActiveTab('monitor')}
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition ${
            activeTab === 'monitor'
              ? 'bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 shadow-sm'
              : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'
          }`}
        >
          <Shield size={14} /> Fleet Compliance Monitor
        </button>
        <button
        ...
        >
          <Clock size={14} /> Rest & Curfew Solver
        </button>
      </div>

      {activeTab === 'calculator' ? (
        <PredictiveRestCalculator />
      ) : (
        <>
