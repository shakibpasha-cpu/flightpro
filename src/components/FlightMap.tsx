...
import { getAirportDetails, searchAirports, searchHandlingAgents } from '../services/aiService';
import { getLiveWeather, getLiveNotams, MetarData, NotamData } from '../services/weatherService';
import { Layers, Map as MapIcon, Compass, Wind, Shield, Info, X, ChevronRight, ChevronDown, GripVertical, ListOrdered, Clock, Trash2, Plus, Globe, Plane, Route, Users, Sparkles, Loader2, Mail, Phone, ExternalLink, Cloud } from 'lucide-react';
            ...
            <>
              ...
              {weather && (
                <div className="mt-2 pt-2 border-t dark:border-gray-700">
                  ...
                  <p className="font-mono text-[10px] text-gray-600 dark:text-gray-400 break-words">{weather.metar}</p>
                </div>
              )}
...
interface Leg {
  ...
  firs?: { name: string, country: string, rules?: string, overflightCharge?: number }[];
  etd?: string;
  eta?: string;
      ...
      if (leg.destination && (!leg.handlingAgents || leg.handlingAgents.length === 0)) {
        ...
        searchHandlingAgents(leg.destination).then(data => {
          if (data.agents) {
            ...
            newLegs[selectedLegIndex] = { ...newLegs[selectedLegIndex], handlingAgents: data.agents };
  ...
  return (
    <div className="relative h-full w-full group/map">
                    ...
                    {localLegs[selectedLegIndex].firs!.map((fir, fidx) => (
                      <div key={fidx} className="p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50 rounded-2xl">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-black text-gray-900 dark:text-white">{fir.name}</span>
                        ...
                        {fir.overflightCharge && (
                          <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                            <span className="text-[9px] font-bold text-gray-400 uppercase">Est. Charge</span>
                            <span className="text-xs font-black text-green-600 dark:text-green-400">${fir.overflightCharge.toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                    ))}
      ...
      >
        ...
        {/* Existing Restricted Area Polygons */}
              ...
              >
                <Popup className="dark-popup">
                  <div className="p-2">
                    ...
                    <div className="mt-2 text-[8px] font-bold text-gray-400 uppercase">
      ...
      {hasLegs ? (
        <>
          ...
          {localLegs!.map((leg, idx) => {
            ...
            const glowColor = hasSafetyIssues ? "rgba(239, 68, 68, 0.3)" : "rgba(99, 102, 241, 0.3)";
